...
function changeHandler(element) {
  ...
  fetch(urlFetchCostAndBudgetlines, {
    ...
    .then((jsonData) => {
      renderOptionsForCostBudgetlines(jsonData, cstCEl, bgtlineEl)
    })
    .catch((err) => {
...
function addNewRow() {
  ...
  fetch(urlFetchClients, {
    ...
    .then((jsonData) => {
      renderPosts(jsonData, selectBlock1)
    })
    .catch((err) => {
  ...
  let userInputs = {
    "client_id": client_id_element_main.value,
    "originalvalue": ""
  };
  fetch(urlFetchCostAndBudgetlines, {
    method: 'POST',
    ...
    .then((jsonData) => {
      renderOptionsForCostBudgetlines(jsonData, selectBlock2, selectBlock3)
    })
    .catch((err) => {
      return console.log("ERROR:", JSON.stringify(err.message));
    });
