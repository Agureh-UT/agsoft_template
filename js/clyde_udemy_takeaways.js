//this doesnt use the formData API it uses the application/x-www-form-urlencoded; charset=UTF-8. Makes use of the new URLSearchParams() api
...
const renderPosts = (jsonData, baseSelectNode) => {
  ...
  // baseSelectNode.dataset.originalvalue = baseSelectNode.dataset.originalvalue!== '' ? baseSelectNode.dataset.originalvalue : '';
...
function changeHandler(element) {
  ...
  //Above FETCH METHOD RETURNS A PROMISE, NOt the data
    .then(res => res.json())
    //Above .json() METHOD RETURNS A PROMISE, that will resolve to a javascript object, so that we can work with the JS object on the browser using javascript. That is why we call below .then() method to access data from the promise
    // the json() method is given to us by the browser not javascript infact the entire fetch() standard is given to us by whatwig and not the ecmascript standard. THe json() method takes a response stream(response object body data), and reads it to completion so basically this is an asynchronous task
    // the json() method is different from JSON.parse() since JSON.parse()is a built in JS object
